...
async function getItems(): Promise<ItemDict> {
  const itemList: ItemDict = await jsObjectFileToJson(
    "https://github.com/MadeBaruna/paimon-moe/raw/main/src/data/itemList.js",
  ) as unknown as ItemDict;
  return itemList;
}
...
async function getDailyFarmableItems(): Promise<FarmableItem[]> {
  ...
  const farmableItemKeys: string[] = Object.keys(itemList).filter((
    key: string,
  ) => itemList[key].day);
  const farmableItems: FarmableItem[] = farmableItemKeys.map((key) =>
    itemList[key]
  ) as FarmableItem[];
